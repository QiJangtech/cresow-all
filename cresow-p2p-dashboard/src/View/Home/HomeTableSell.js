...
class HomeTableSell extends Component {
  render() {
    const {
              ...
              <tbody>
                {data.map((app,index) => (
                  <tr
                    key={index}
                  >
                    <td><span className="sell-price">{app.conversion_rate}</span></td>
                    <td>{app.coins_left}</td>
                    <td></td>
                    <td></td>
                    <td><span className="sell-action"><Link to={`/sell/${app.id}`}>Sell Now</Link></span></td> 
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <PaginationContainer tableName="HomeTableSell"/>
        </BoxBody>
      </Box>
    );
  }
}
HomeTableSell.propTypes = propTypes;
export default sematable('HomeTableSell', HomeTableSell, columns, configs);