Doorkeeper.configure do
  ...
  resource_owner_from_credentials do |routes|
    if params[:scope].present?
      case params[:scope]
      when "user" 
        u = User.find_for_database_authentication(:email => params[:username], :disabled => false)
        if u.blank?
          u = User.find_for_database_authentication(:contact_no => params[:username], :disabled => false)
        end
      end
    else
      u = User.find_for_database_authentication(:email => params[:username], :disabled => false)
