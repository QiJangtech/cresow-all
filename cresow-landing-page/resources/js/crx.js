var crxChart = [
    {
      value: 5,
      label: 'Bounty',
      color: 'l(1, 1, 0, 0)#ff5502-#ffff00'
    },
    {
      value: 15,
      label: 'Future',
      color: 'l(0, 1.1, 1, 1)#03fdf4-#07265f'
    },
    {
      value: 20,
      label: 'Core Team',
      color: 'l(1,1,0,1)#78ff00-#068941'
    },
    {
      value: 65,
      label: 'Public Sale',
      color: 'l(1,0.8,1,0)#2a0954-#f42ab4'
    },
];
...
(function ($, doc, win, Snap) {
    ...
    Donut.prototype.startShowAnimationOneEntry = function (d, percentStart, percentEnd, center, radius, callback) {
        ...
        this.animateArcByDegrees(d.arc, 100, callback);
    ...
    Donut.prototype.normalElement = function (d) {
        ...
        this.animateArcByStroke(d.arc, d.arc.strokeWidth + 6, d.arc.strokeWidth, 500, function(){
          d.arc.draw.mouseover(function () {
            ...
            updateCurrentValue(d, $(s.node).next().find('.value'));
          });
...
crxDonut.startShowAnimation(function(){
    $('button').attr('disabled', false);
});

btcDonut.startShowAnimation(function(){
    $('button').attr('disabled', false);
});

function updateCurrentValue(d, elem){
    $(elem).text(d.value)
    $(elem).closest('.chart').next().find('.name:contains("'+d.label+'")').closest('li').addClass('active');
    $(elem).closest('.chart').next().find('.name:contains("'+d.label+'")').closest('li').siblings().removeClass('active');
}

$( document ).ready(function() {
    $('.crx-chart-list>li').hover(
        function() {
            var $p=$(this).parent().children();
            var $index = $p.index(this);
            var d = crxDonut.opts.data[$index];
            updateCurrentValue(d, $(this).parent().prev().find('.current-value>.value'))
            crxDonut.hightlightElement(d);
        }
    )
    $('.btc-chart-list>li').hover(
        function() {
            var $p=$(this).parent().children();
            var $index = $p.index(this);
            var d = btcDonut.opts.data[$index];
            updateCurrentValue(d, $(this).parent().prev().find('.current-value>.value'))
            btcDonut.hightlightElement(d);
        }
    )
});